...
c      do i=1-mbc,mx+mbc
c         xlow = xlower+dble(i-1)*dx
c         if (xlow+0.5d0*dx .lt. RC) then
c             do j=my+mbc,1-mbc,-1
c                ylow = ylower+dble(j-1)*dy
c                call cellave(xlow,ylow,dx,dy,wlin)
c
c                if (wlin .ne. 1.0d0 .and.
c     &              wlin .ne. 0.0d0) then
c                   # find the interface cell
c
c                    jk = j
c                    yk = ylower+dble(jk-1)*dy+0.5d0*dy
c
c                    p1 = p0 
c
c                    do jj=jk-1,1-mbc,-1
c                       yc = ylower+dble(jj-1)*dy+0.5d0*dy
c
c                       p2 = p0-rho2*grav*(yc-yk)
c
c                       call prmtoc(qloc,rho1,rho2,vx2,vy2,p1,p2,
c     &                   zf(2),1.0d0-zf(2))
c
