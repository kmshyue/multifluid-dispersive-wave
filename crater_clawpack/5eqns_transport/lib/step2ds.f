...
c ----------------------------------------------------------------
c
      subroutine step2ds(maxm,maxmx,maxmy,meqn,mwaves,maux,mbc,
     &           mx,my,q,qold,aux,auxold,
...
c        # This shouldn't happen due to checks in claw2
         write(6,*) '*** not enough work space in step2ds'
...
c            # with capa array.  
             do m=1,meqn
                do i=1,mx
                   q(i,j,m) = q(i,j,m)+qadd(i,m)-
     &                dtdx*(fadd(i+1,m)-fadd(i,m))/aux(i,j,mcapa)
                   enddo
                enddo
         endif
         enddo    
...
c            # with capa array.  
             do m=1,meqn
                do j=1,my
                   q(i,j,m) = q(i,j,m)+qadd(j,m)-
     &                dtdy*(fadd(j+1,m)-fadd(j,m))/aux(i,j,mcapa)
                   enddo
                enddo
         endif
         enddo
